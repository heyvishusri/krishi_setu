...
const Weather = () => {
  ...
  const fetchWeatherData = async ({ city = null, lat = null, lon = null }) => {
    if (!API_KEY) {
      setError(
        "API key is missing. Please configure your OpenWeatherMap API key in the environment variables."
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!forecastResponse.ok) {
        ...
        throw new Error(
          `Forecast Error: ${errorData.message || forecastResponse.statusText}`
        );
      }
      ...
      setError(
        err.message ||
          "Failed to fetch weather data. Check city name or network."
      );
    } finally {
      setLoading(false);
    }
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setError(
            `Geolocation Error: ${err.message}. Please ensure location access is granted.`
          );
          setLoading(false);
        }
      );
