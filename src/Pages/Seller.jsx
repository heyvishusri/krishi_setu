  ...
  useEffect(() => {
    const fetchAllPosts = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${BACKEND_URL}/api/seller-posts`);
        setPostsData(response.data);
      } catch (err) {
        console.error("Error fetching all seller posts:", err);
        setError(
          err.response?.data?.message || err.message || "Failed to load posts"
        );
        toast.error("Could not load seller posts.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-4">
      <h1 className="mb-6 text-3xl font-bold text-center text-gray-800">
        Available Produce from Sellers
      </h1>{" "}
      {/* Update Title */}
      ...
      {!loading &&
        ...
        (postsData.length === 0 ? (
          <div className="text-center text-gray-500">
            No seller posts currently available.
          </div>
        ) : (
            ...
            {postsData.map((post) => (
              ...
              >
                ...
                <div className="pt-3 mt-auto space-y-1 text-sm text-gray-700 border-t">
                  ...
                  <p>
                    <strong>Price:</strong> {post.pricePerUnit}
                  </p>
                  <p>
                ...
                {post.user?.mobile && (
                  <div className="flex items-center justify-center pt-3 mt-4 border-t">
                    ...
                    >
                      ...
                      Contact Seller ({post.user.mobile})
                    </a>
                  </div>
                )}
              </div>
